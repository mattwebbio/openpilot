...
class CarState(CarStateBase, CarStateExt):
  def __init__(self, CP, CP_SP):
    ...
    self.car_cruise_speed = 0
    self.sm = messaging.SubMaster(['llmDecision'])
    self.last_llm_frame = -1
  ...
  def update(self, can_parsers) -> tuple[structs.CarState, structs.CarStateSP]:
    ...
    self.car_cruise_speed = ret.cruiseState.speed
    self.sm.update(0)
    llmd = self.sm["llmDecision"]
    if llmd.frame != self.last_llm_frame:
      self.last_llm_frame = llmd.frame
      ret.cruiseState.speed = llmd.predictedSpeed
    elif llmd.frame == self.last_llm_frame:
      ret.cruiseState.speed = self.car_cruise_speed
